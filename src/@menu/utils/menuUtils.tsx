...
export const confirmUrlInChildren = (children: ChildrenType['children'], url: string): boolean => {
  ...
  if (isValidElement(children)) {
    const { component, href, exactMatch, activeUrl, children: subChildren } = (children as ReactElement<any>).props

    if (component && (component as ReactElement<any>).props.href) {
      return exactMatch === true || exactMatch === undefined
        ? (component as ReactElement<any>).props.href === url
...
const processMenuChildren = (children: ReactNode, mapFunction: (child: ReactNode) => ReactNode): ReactNode => {
  return Children.map(children, child => {
    ...
    return (child as ReactElement<any>).props?.menuData ? (
      <GenerateVerticalMenu menuData={(child as ReactElement<any>).props.menuData} />
    ) : (
      mapFunction(child)
    )
  })
...
export const mapHorizontalToVerticalMenu = (children: ReactNode): ReactNode => {
  return Children.map(children, child => {
    ...
    const { children: childChildren, verticalMenuProps, ...rest } = (child as ReactElement<any>).props
